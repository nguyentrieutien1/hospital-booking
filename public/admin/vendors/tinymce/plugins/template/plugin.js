...
(function () {
    ...
    var insertTemplate = function (editor, _ui, html) {
        ...
        global$1.each(dom.select("*", el), function (n) {
            if (
                hasClass(n, getCreationDateClasses(editor).replace(/\s+/g, "|"))
            ) {
            ...
            if (
                hasClass(
                    n,
                    getSelectedContentClasses(editor).replace(/\s+/g, "|")
                )
            ) {
                n.innerHTML = sel;
            }
                ...
                if (dom.hasClass(e, "mceTmpl")) {
                    global$1.each(dom.select("*", e), function (e) {
                        if (
                            dom.hasClass(
                                e,
                                getModificationDateClasses(editor).replace(
                                    /\s+/g,
                                    "|"
                                )
                            )
        ...
        var onChange = function (templates, updateDialog) {
            return function (api, change) {
                if (change.name === "template") {
                    ...
                    findTemplate(templates, newTemplateTitle).each(function (
                        t
                    ) {
                        api.block("Loading...");
